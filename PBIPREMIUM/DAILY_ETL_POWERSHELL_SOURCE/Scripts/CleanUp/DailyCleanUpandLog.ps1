...
C:\Windows\azcopy\azcopy.exe copy "C:\Program Files\WindowsPowerShell\Logs\PBIPREMIUMLOGS\Master\PBIPREMIUMLOGS_$currenttimestamp.csv" "https://test.dfs.core.windows.net/staging/Logs/PBIPREMIUM/Powershell/PBIPREMIUMLOGS_$currenttimestamp.csv" --overwrite=FALSE --follow-symlinks --recursive --from-to=LocalBlobFS --put-md5
C:\Windows\azcopy\azcopy.exe copy "C:\Program Files\WindowsPowerShell\Logs\PBIAPILOGS\Master\PBIAPILOGS_$currenttimestamp.csv" "https://test.dfs.core.windows.net/staging/Logs/PBIAPI/Powershell/PBIAPILOGS_$currenttimestamp.csv" --overwrite=FALSE --follow-symlinks --recursive --from-to=LocalBlobFS --put-md5
