...
$tableName = "DQLCMetrics"
$datalakeFolder = "DQLC_Metrics"
...
$query = ComputeNewDataCheckQuery DQLCMetrics
...
Export-PbiPremiumData "EVALUATE CALCULATETABLE (Filter(DQLCMetrics,DATEDIFF(DQLCMetrics[timestamp],TODAY(),DAY)=1))" "$file_name" $folderFullPath $auth
}
catch
{Write-Log -Message $_.Exception.Message -Type ($_.Exception.GetType().FullName) }
