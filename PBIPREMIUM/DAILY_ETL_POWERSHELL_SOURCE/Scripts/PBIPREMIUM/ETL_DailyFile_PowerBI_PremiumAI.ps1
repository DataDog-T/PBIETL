...
$query = ComputeNewDataCheckQuery AIExecution
...
Export-PbiPremiumData "EVALUATE CALCULATETABLE (Filter(AIExecution, DATEDIFF(AIExecution[timestamp],TODAY(),DAY)=1))" "$file_name" $folderFullPath $auth
}
catch
{Write-Log -Message $_.Exception.Message -Type ($_.Exception.GetType().FullName) }
