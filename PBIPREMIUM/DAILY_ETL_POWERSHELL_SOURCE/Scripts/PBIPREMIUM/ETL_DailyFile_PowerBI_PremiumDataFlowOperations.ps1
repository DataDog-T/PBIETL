...
$tableName = "Dataflow_Metrics"
$datalakeFolder = "Dataflow_Metrics"
...
$query = ComputeNewDataCheckQuery DataflowOperations
...
Export-PbiPremiumData "EVALUATE CALCULATETABLE (Filter(DataflowOperations, DATEDIFF(DataflowOperations[timestamp],TODAY(),DAY)=1))" "$file_name" $folderFullPath $auth
}
catch
{Write-Log -Message $_.Exception.Message -Type ($_.Exception.GetType().FullName) }
