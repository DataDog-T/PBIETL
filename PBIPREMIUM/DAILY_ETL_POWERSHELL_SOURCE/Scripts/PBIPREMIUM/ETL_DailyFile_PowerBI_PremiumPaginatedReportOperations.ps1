...
$tableName = "PaginatedReport_Metrics"
$datalakeFolder = "PaginatedReports_Metrics"
...
Export-PbiPremiumData "EVALUATE CALCULATETABLE (Filter(PaginatedReportUsage,DATEDIFF(PaginatedReportUsage[timestamp],TODAY(),DAY)=1))" "$file_name" $folderFullPath $auth
}
catch
{Write-Log -Message $_.Exception.Message -Type ($_.Exception.GetType().FullName) }

Write-Log -Message Write-Log -Message ("Initiating Export-PbiPremiumData Function from Module PBIPREMIUM" + $sourcemetrics.'[Hours]' + " " + "exist of use for yesterday") -Type "CommandEnd" -Type "CommandEnd"
