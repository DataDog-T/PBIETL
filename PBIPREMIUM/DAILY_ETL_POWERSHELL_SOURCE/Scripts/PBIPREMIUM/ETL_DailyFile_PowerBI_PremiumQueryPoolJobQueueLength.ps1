...
$tableName = "QueryPoolJobQueueLength"
$datalakeFolder = "QueryPoolJobQueueLength"
...
Export-PbiPremiumData "EVALUATE CALCULATETABLE (Filter(QueryPoolJobQueueLength, DATEDIFF(QueryPoolJobQueueLength[timestamp],TODAY(),DAY)=1))" "$file_name" $folderFullPath $auth
}
catch
{Write-Log -Message $_.Exception.Message -Type ($_.Exception.GetType().FullName) }
