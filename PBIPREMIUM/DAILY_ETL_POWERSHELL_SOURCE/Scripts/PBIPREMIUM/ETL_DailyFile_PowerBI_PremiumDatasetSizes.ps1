...
$tableName = "DatasetSizes"
$datalakeFolder = "DatasetSizes"
...
Export-PbiPremiumData "EVALUATE CALCULATETABLE (Filter(DatasetSize,DATEDIFF(DatasetSize[timestamp],TODAY(),DAY)=1))" "$file_name" $folderFullPath $auth
}
catch
{Write-Log -Message $_.Exception.Message -Type ($_.Exception.GetType().FullName) }
