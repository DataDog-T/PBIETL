...
$tableName = "InactiveMemory_Metrics"
...
Export-PbiPremiumData "EVALUATE CALCULATETABLE (Filter(InactiveMemoryMetrics,DATEDIFF(InactiveMemoryMetrics[timestamp],TODAY(),DAY)=1))" "$file_name" $folderFullPath $auth
}
catch
{Write-Log -Message $_.Exception.Message -Type ($_.Exception.GetType().FullName) }
