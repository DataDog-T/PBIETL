...
$tableName = "Eviction_Metrics"
$datalakeFolder = "Eviction_Metrics"
...
$query = ComputeNewDataCheckQuery EvictionMetrics
...
Export-PbiPremiumData "EVALUATE CALCULATETABLE (Filter(EvictionMetrics,DATEDIFF(EvictionMetrics[timestamp],TODAY(),DAY)=1))" "$file_name" $folderFullPath $auth
}
catch
{Write-Log -Message $_.Exception.Message -Type ($_.Exception.GetType().FullName) }
