...
$tableName = "Workload_Settings"
$datalakeFolder = "Workload_Settings"
...
Export-PbiPremiumData "EVALUATE CALCULATETABLE (WorkloadResourceSettings)" "$file_name" $folderFullPath $auth
}
catch
{Write-Log -Message $_.Exception.Message -Type ($_.Exception.GetType().FullName) }
