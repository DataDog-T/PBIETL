...
$query = ComputeNewDataCheckQuery QueryMetrics
...
Export-PbiPremiumData "EVALUATE CALCULATETABLE (Filter(QueryMetrics, DATEDIFF(QueryMetrics[timestamp],TODAY(),DAY)=1))" "$file_name" $folderFullPath $auth
}
catch
{Write-Log -Message $_.Exception.Message -Type ($_.Exception.GetType().FullName) }
