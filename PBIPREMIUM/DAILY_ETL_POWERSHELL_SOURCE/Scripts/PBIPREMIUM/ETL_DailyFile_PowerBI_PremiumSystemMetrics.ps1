...
$query = ComputeNewDataCheckQuery SystemMetrics
...
Export-PbiPremiumData "EVALUATE CALCULATETABLE (Filter(SystemMetrics,DATEDIFF(SystemMetrics[timestamp],TODAY(),DAY)=1))" "$file_name" $folderFullPath $auth
}
catch
{Write-Log -Message $_.Exception.Message -Type ($_.Exception.GetType().FullName) }
