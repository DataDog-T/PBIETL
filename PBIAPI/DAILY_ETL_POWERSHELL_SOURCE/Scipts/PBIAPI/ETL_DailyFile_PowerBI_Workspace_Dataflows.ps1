...
$tableName = "Workspace_Dataflows"
$datalakeFolder = "Workspace_Dataflows"
...
$LASTAPIPARAM = '/dataflows' #Parameter needed to retrieve all datasets in a workspace
$folderPath = "C:\Users\bvadmin\PBIAPI\Workspacefordataflows_$dailyDateofData.csv" #Path on VM to save results to
...
$MAINURL  = 'https://api.powerbi.com/v1.0/myorg/admin/groups/' + $Id.Id + $LASTAPIPARAM
...
$allinfo | foreach-Object { $_.value} | Select-Object *, @{l="dataflowid";e={$Id.Id}} | Export-Csv $Fullpath -Append
}
catch
{ Write-Log -Message $_.Exception.Message -Type ($_.Exception.GetType().FullName) }
