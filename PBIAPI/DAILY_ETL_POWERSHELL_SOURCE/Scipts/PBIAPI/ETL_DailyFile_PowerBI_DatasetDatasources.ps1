...
$tableName = "Dataset_Datasources"
$datalakeFolder = "Dataset_Datasources"
...
$folderPath = "C:\Users\bvadmin\PBIAPI\DatasetsforDatasources_$dailyDateofData.csv" #Path on VM to save results to
...
$datasets | foreach-Object { $_.value } | Select-Object | Export-Csv -path $folderPath
}
catch
{ Write-Log -Message $_.Exception.Message -Type ($_.Exception.GetType().FullName) }
...
$Uri  = 'https://api.powerbi.com/v1.0/myorg/admin/datasets/' + $Id.Id + '/datasources'
 $allinfo=Invoke-PowerBIRestMethod -Url $Uri -Method Get -ErrorAction SilentlyContinue | ConvertFrom-Json 
$allinfo | foreach-Object { $_.value} | Select-Object *,@{l="datasetId";e={$Id.Id}} | Export-Csv $Fullpath -Append 
}
catch
{ Write-Log -Message $_.Exception.Message -Type ($_.Exception.GetType().FullName) }
