...
$tableName = "Dataflow_Datasources"
$datalakeFolder = "Dataflow_Datasources"
...
$LASTAPIPARAM = '/datasources' #Parameter needed to retrieve all datasets in a workspace
$folderPath = "C:\Users\bvadmin\PBIAPI\DataflowDevelopersforData_$dailyDateofData.csv" #Path on VM to save results to
...
$Uri  = 'https://api.powerbi.com/v1.0/myorg/admin/dataflows/' + $Id.ObjectId + '/datasources'
 $allinfo=Invoke-PowerBIRestMethod -Url $Uri -Method Get -ErrorAction SilentlyContinue | ConvertFrom-Json 
$allinfo | foreach-Object { $_.value} | Select-Object *,@{l="dataflowid";e={$Id.ObjectId}} | Export-Csv $Fullpath -Append
}
catch
{ Write-Log -Message $_.Exception.Message -Type ($_.Exception.GetType().FullName) }
